...
def print_nan_stats(in_arr):
    ...
    print(
        "[0.0: {:.5f}, 1.0: {:.5f}, NAN: {:.5f}]".format(
            np.sum(in_arr == 0.0) / in_arr.size,
            np.sum(in_arr == 1.0) / in_arr.size,
...
def time_window_mean_std_trend(in_arr):
    ...
    trend_vect = np.apply_along_axis(
        lambda slice_arr: mlhc_statistics.lr_slope(slice_arr), 0, in_arr
    ).flatten()
...
def time_window_5point_summary(in_arr):
    ...
    if in_arr.size == 0:
        ...
        trend_vect = np.apply_along_axis(
            lambda slice_arr: first_last_diff(slice_arr), 0, in_arr
        ).flatten()
...
def time_window_5point_summary_NAN(in_arr):
    ...
    if in_arr.size == 0:
        ...
        trend_vect = np.apply_along_axis(
            lambda slice_arr: first_last_diff_NAN(slice_arr), 0, in_arr
        ).flatten()
