    ...
    def transform(self, patient_df, pid=None):
        ...
        for var_idx,variable in enumerate(all_keys):
            ...
            if variable[0]=="v":
                ...
                if self.use_adaptive_impute:
                    ...
                    if variable in ["v1000","v1010","v10020000","v30005010","v30005110","vm13","vm24","vm31","vm32"]:
                        ...
                        est_vals,cum_count_ts,time_to_last_ms=bern_forward_fill.impute_forward_fill_adaptive(observ_ts,observ_val,time_grid, global_impute_val, self.grid_period,
                        ...
                        est_vals,cum_count_ts,time_to_last_ms=bern_forward_fill.impute_forward_fill_adaptive(observ_ts, observ_val, time_grid, global_impute_val, self.grid_period,var_type="weight")
                    else:
                        est_vals,cum_count_ts,time_to_last_ms=bern_forward_fill.impute_forward_fill_adaptive(observ_ts,observ_val,time_grid, global_impute_val, self.grid_period,
                                                                                fill_interval_secs= fill_interval_secs, rolling_mean_secs= rolling_mean_secs, 
                                                                                return_mean_secs= return_mean_secs, var_type="non_pharma", var_encoding=var_encoding,
            ...
            #  is 0 (no medication flow)
                ...
                assert(np.isfinite(observ_val).all())
                assert(np.isfinite(observ_ts).all())
                est_vals,cum_count_ts,time_to_last_ms=bern_forward_fill.impute_forward_fill_adaptive(observ_ts, observ_val, time_grid, global_impute_val, self.grid_period,var_type="pharma")
