...
class AllLabel:
    ...
    def transform(self, df_pat, df_endpoint, pid=None):
        ...
        if df_pat.shape[0] == 0 or df_endpoint.shape[0] == 0:
            print(
                "WARNING: Patient {} has no impute data, skipping...".format(pid),
                flush=True,
            )
            return None
        ...
        try:
            if self.dataset == "bern":
                df_endpoint = df_endpoint.reindex(
                    index=df_pat.AbsDatetime, method="nearest"
                )
            elif self.dataset == "mimic":
                df_endpoint = df_endpoint.reindex(
                    index=df_pat.AbsDatetime, method="ffill"
                )

        except:
            print(
                "WARNING: Issue when re-indexing frame of patient: {}".format(pid),
                flush=True,
            )
            return None
        ...
        worse_state_arr = bern_labels.future_worse_state(
            endpoint_status_arr, self.lhours, self.rhours, self.grid_step_seconds
        )  # Joint (A|D|E)
        worse_state_soft_arr = bern_labels.future_worse_state_soft(
            endpoint_status_arr, self.lhours, self.rhours, self.grid_step_seconds
        )  # Joint (B|C|D|E)
        from_0_arr = bern_labels.future_worse_state_from_0(
            endpoint_status_arr, self.lhours, self.rhours, self.grid_step_seconds
        )  # Separate A
        from_0_soft_arr = bern_labels.future_worse_state_soft_from_0(
            endpoint_status_arr, self.lhours, self.rhours, self.grid_step_seconds
        )  # Separate B
        from_probably_not_arr = bern_labels.future_worse_state_from_pn(
            endpoint_status_arr, self.lhours, self.rhours, self.grid_step_seconds
        )  # Separate C
        from_1_arr = bern_labels.future_worse_state_from_1(
            endpoint_status_arr, self.lhours, self.rhours, self.grid_step_seconds
        )  # Separate D
        from_2_arr = bern_labels.future_worse_state_from_2(
            endpoint_status_arr, self.lhours, self.rhours, self.grid_step_seconds
        )  # Separate E;
        from_1_or_2_arr = bern_labels.future_worse_state_from_1_or_2(
            endpoint_status_arr, self.lhours, self.rhours, self.grid_step_seconds
        )  # Join(D|E)
        ...
        time_to_worse_state_binned_arr = bern_labels.time_to_worse_state_binned(
            endpoint_status_arr, self.lhours, self.rhours, self.grid_step_seconds
        )
        time_to_worse_state_arr = bern_labels.time_to_worse_state(
            endpoint_status_arr, self.lhours, self.rhours, self.grid_step_seconds
        )
        ...
        output_df_dict["WorseState{}To{}Hours".format(self.lhours, self.rhours)] = (
            worse_state_arr
        )
        output_df_dict["WorseStateSoft{}To{}Hours".format(self.lhours, self.rhours)] = (
            worse_state_soft_arr
        )
        output_df_dict[
            "WorseStateFromZero{}To{}Hours".format(self.lhours, self.rhours)
        ] = from_0_arr
        output_df_dict[
            "WorseStateSoftFromZero{}To{}Hours".format(self.lhours, self.rhours)
        ] = from_0_soft_arr
        output_df_dict[
            "WorseStateFromPn{}To{}Hours".format(self.lhours, self.rhours)
        ] = from_probably_not_arr
        output_df_dict[
            "WorseStateFromOne{}To{}Hours".format(self.lhours, self.rhours)
        ] = from_1_arr
        output_df_dict[
            "WorseStateFromTwo{}To{}Hours".format(self.lhours, self.rhours)
        ] = from_2_arr
        output_df_dict[
            "WorseStateFromOneOrTwo{}To{}Hours".format(self.lhours, self.rhours)
        ] = from_1_or_2_arr
        ...
        output_df_dict[
            "TimeToWorseState{}To{}Hours".format(self.lhours, self.rhours)
        ] = time_to_worse_state_arr
        output_df_dict[
            "TimeToWorseStateBinned{}To{}Hours".format(self.lhours, self.rhours)
        ] = time_to_worse_state_binned_arr
...
class DeteriorationLabel:
    ...
    def transform(self, df_pat, df_endpoint, pid=None):
        ...
        if self.lhours == 0:
            label_arr = bern_labels.future_worse_state(
                event1_arr,
                event2_arr,
                event3_arr,
                maybe1_arr,
            ...
            label_arr = bern_labels.future_deterioration(
                event1_arr,
                event2_arr,
                event3_arr,
                maybe1_arr,
...
class WorseStateLabel:
    def __init__(self, lhours, rhours):
        ...
        self.lhours = lhours
        self.rhours = rhours
        self.label_key = "WorseState_{}To{}Hours".format(
            float(self.lhours), float(self.rhours)
        )
        self.grid_step_seconds = 300.0
...
class WorseStateSoftLabel:
    ...
    def transform(self, df_pat, df_endpoint, pid=None):
        ...
        # Any deterioration, does not require that the exact downward takes place in the forward horizon, but only if there
        # is some more severe endpoint in the period
        label_arr = bern_labels.future_worse_state_soft(
            event1_arr,
            event2_arr,
            event3_arr,
            maybe1_arr,
