...
if v5:
    ...
    excel_path = os.path.abspath("./preprocessing/resource/varref_excel_v6.tsv")
    derived_path = os.path.abspath("./preprocessing/resource/pharma_acting_period.npy")
    meta_derived_path = os.path.abspath(
...
if v5:
    ...
    drugs["Pharma acting period"] = list(
        map(lambda x: enum_ref[x], drugs["Pharma acting period"].values)
    )
    drugs.set_index("ID", inplace=True)
    pID_to_period = drugs["Pharma acting period"].to_dict()
else:
    drugs["VariableID"] = list(
        map(lambda x: "p" + str(int(x)), drugs["VariableID"].values)
    )
    drugs_meta["MetaVariableID"] = list(
        map(lambda x: "pm" + str(int(x)), drugs["MetaVariableID"].values)
    )
    drugs["PharmaActingPeriod"] = list(
        map(lambda x: enum_ref[x], drugs["PharmaActingPeriod"].values)
    )
    drugs_meta["PharmaActingPeriod"] = list(
        map(lambda x: enum_ref[x], drugs_meta["PharmaActingPeriod"].values)
    )
