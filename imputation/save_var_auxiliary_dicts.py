...
def save_var_auxiliary_dicts(configs):
    df_varref = pd.read_csv(
        configs["current_varref_table"], delimiter="\t", encoding="latin"
    )
    ...
    for var in normal_vars:
        ...
        if mlhc_math.is_numeric(normal_val):
            ...
            # Encode string category with the correct bit pattern
                ...
                normal_dict["v{}".format(var)] = configs["category_str_to_num"][
                    normal_val
                ]
    ...
    df_labref = pd.read_csv(
        configs["current_labref_table"], delimiter="\t", encoding="latin"
    )
    ...
    for var in meta_normal_vars:
        ...
        if mlhc_math.is_numeric(normal_val):
            ...
            meta_normal_dict["{}m{}".format(var_prefix, var)] = configs[
                "category_str_to_num"
            ][normal_val]
        elif normal_val in ["none", "no"]:
...
def parse_cmd_args():
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Should script generate output on FS?",
    )
